...
DATASET_CONFIGS = {
    ...
    "DOR": dict(
        ...
        camera="AuricamD80",
        #DOR dataset specific parameters
        canny_low = 60, 
        curvature_window=11, 
    ),
    "CPO_scaled": dict(
...
if __name__ == "__main__":
    ...
    # dataset_dir = Path(r"DOR_dataset\rendering")
    # output_dir  = Path(r"results/updated_model_DOR_20072026/DOR_dataset")
    ...
    if cfg.get("frame_prefix") is not None:
        ...
        # files = [f for f in files if int(pattern.match(f.stem).group(1)) > 14]
    ...
    # files = [f for f in files if (s := extract_scene_number(f)) is not None and s > 163]  # Only run for scene > x
    # files = [f for f in files if (m := re.search(r'WCAM1_AURICAM_(\d+)', f.stem)) and int(m.group(1)) > 158 and int(m.group(1)) < 300]
